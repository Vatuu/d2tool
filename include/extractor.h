...
namespace d2::extract {
    ...
    private:
        ...
        bool initOodle();
        bool decompress(Block *b, std::vector<byte> source, std::vector<byte> target) const;
        bool decrypt(Block *b, const byte* nonce, std::vector<byte>& source, std::vector<byte>& buffer);
